...
class ProcessingEngine:
    ...
    def process_image(self, image_file_path: Union[str, Path]):
        inputs, outputs = self._prepare_inputs_and_outputs_for_inference(file_path=image_file_path, pipeline=IMAGE_PIPELINE)

        # Run the inference
        result = self.client.infer(model_name=IMAGE_PIPELINE['model'], inputs=inputs, outputs=outputs)
        embedding = result.as_numpy(IMAGE_PIPELINE['outputs'][0])
        return embedding

    def process_video(self, video_file_path: Union[str, Path], grid=False) -> Tuple:
        ...
        inputs, outputs = self._prepare_inputs_and_outputs_for_inference(file_path=video_file_path)
        ...
        self.client.async_stream_infer(model_name=model, inputs=inputs, request_id="0", outputs=outputs, enable_empty_final_response=True)
        ...
        while True:
            ...
            for i in range(len(embeddings)):
                image = Image.fromarray(snippets[i].transpose(1, 2, 0).astype(np.uint8), 'RGB')
    ...
    @staticmethod
    def search_embeddings(query:np.ndarray, index_path: Union[str, Path], k=100):
    ...
    @staticmethod
    def _prepare_inputs_and_outputs_for_inference(file_path: Union[str, Path], pipeline=VIDEO_PIPELINE) -> Tuple:
        buffer = np.fromfile(file_path, dtype=np.uint8)
        buffer = np.expand_dims(buffer, axis=0)
        input_shape = list(buffer.shape)
        inputs, outputs = [], []
        inputs.append(grpcclient.InferInput(pipeline['input'], input_shape, pipeline['input_type']))
